...
from app.facade.helper import (request_header, session, market_num_dict,
                               get_code_id, trans_num, trade_detail_dict, trade_detail_dict_v2)
...
# 获取单个或多个证券的最新行情指标
def stock_realtime(code_list) -> pd.DataFrame:
    """
    获取股票、期货、债券的最新行情指标
    ...
    fields = ",".join(trade_detail_dict_v2.keys())
    params = (
        ('OSVersion', '14.3'),
        ('appVersion', '6.3.8'),
        ('fields', fields),
    ...
    if not rows:
        df = pd.DataFrame(columns=trade_detail_dict_v2.values())
    else:
        df = pd.DataFrame(rows)[list(trade_detail_dict_v2.keys())].rename(columns=trade_detail_dict_v2)
    df['stores'] = df['编号'].apply(lambda x: market_num_dict.get(str(x)))
    del df['编号']
    df['datetime'] = df['更新时间戳'].apply(lambda x: str(datetime.fromtimestamp(x)))
    del df['更新时间戳']
    # 将object类型转为数值型
    ignore_cols = ['name', 'code', 'stores', 'datetime']
    df = trans_num(df, ignore_cols)
    return df
...
def latest_trade_date():
    date = stock_realtime('上证指数')['datetime'].values[0][:10]
    return date
...
# 获取单只或多只证券（股票、基金、债券、期货(国内))的历史K线数据
