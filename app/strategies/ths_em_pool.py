...
from app.facade.helper import trans_num, ths_header
...
def stock_strong_pool(date=None):
    ...
    old_cols = ['序号', '代码', '_', '名称', '最新价', '涨停价', '_', '涨跌幅',
                ...
                '量比', '涨速', '涨停统计', '所属行业', ]
    temp_df.columns = old_cols
    temp_df['涨停统计'] = temp_df['涨停统计'].apply(lambda x: dict(x)['days']).astype(str) + "/" + temp_df[
        '涨停统计'].apply(lambda x: dict(x)['ct']).astype(str)
