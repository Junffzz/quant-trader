...
from app.constants import Direction, Offset, OrderType, TradeMode, OrderStatus, TradeMarket, OrderTimeInForce
from app.domain.data import Bar, Quote, CapitalDistribution
from app.domain.deal import Deal
from app.domain.order import Order
from app.domain.portfolio import Portfolio
from app.domain.position import PositionData, Position
from app.domain.security import Stock
from app.domain.order import OrderBook
from app.domain.data import _get_data
from app.utils import logger
from trader_config import DATA_MODEL, DATA_PATH
from trader_config import ACTIVATED_PLUGINS
from app.gateways import BaseGateway

PERSIST_TIME_INTERVAL = 5  # 秒
...
class Engine:
    ...
    def __init__(self,
                 ...
                 init_account_balance: Dict[str, AccountBalance] = None,
                 ):
        self.gateways = gateways
        self.plugins = dict()
        for plugin in ACTIVATED_PLUGINS:
            self.plugins[plugin] = importlib.import_module(f"app.plugins.{plugin}")
        # 如果数据库没有启动，则不进行任何持久化操作
        if "mariadb" not in self.plugins:
            return
        ...
        if self.gateways_persist:
            DB = getattr(self.plugins["mariadb"], "DB")
            self.db = DB()
            # 新建线程，定期将数据持久化到数据库
        ...
        self.portfolios = {}
        self.init_portfolio({
            gateway_name: AccountBalance(cash=0)
            for gateway_name in self.gateways
        })
    ...
    def stop(self):
        ...
        if self.has_db():
            ...
            self._persist_t.join((PERSIST_TIME_INTERVAL + 2) * 1000)  # wait for the persist thread stop
            self.db.close()
        logger.info("Engine stops")

    def update_strategy(self, strategy_account: str, strategy_version: str):
        """初始化投资组合相关信息，初始资金要指定不同的gateway"""
        self.strategy_account = strategy_account
        self.strategy_version = strategy_version

    def init_portfolio(self, init_strategy_balance: Dict[str, AccountBalance]):
        """初始化投资组合相关信息，初始资金要指定不同的gateway"""
        # 先初始化投资组合管理，account_balance和position会在后面进行同步sync
        for gateway_name in self.gateways:
            gateway = self.gateways[gateway_name]
            balance = init_strategy_balance[gateway_name]
            portfolio = Portfolio(
                account_balance=balance,  # 账户余额
                position=Position(),  # 仓位管理
                market=gateway  # 交易通道gateway
            )
    ...
    def get_db_order(self, balance_id: int, broker_order_id: str) -> Order:
        ...
        assert order_df.shape[0] == 1, (
            f"There are more than one rows in order: balance_id={balance_id} broker_order_id="
            f"{broker_order_id}")
        order = Order(
    ...
    def get_db_balance(self, strategy_account: str, strategy_version: str, gateway_name: str) -> AccountBalance:
        ...
        broker_account = self.gateways[gateway_name].broker_account
        broker_environment = self.gateways[gateway_name].trade_mode.name
        trade_market = self.gateways[gateway_name].trade_market.value
        balance_df = self.db.select_records(
            table_name="balance",
            broker_name=broker_name,
            broker_environment=broker_environment,
        ...
        if balance_df.empty:
            return None
        assert balance_df.shape[
                   0] == 1, f"There are more than one rows in balance: {self.strategy_account} {self.strategy_version}"
    ...
    def sync_broker_balance(self, gateway_name: str):
        ...
        if balance_df.empty:
            account_ids = self.db.select_records(table_name="balance",
                                                 columns=["broker_account_id", "strategy_account_id"])
            ...
            self.db.insert_records(
                ...
                cash=cash,
                cash_by_currency=broker_balance.cash_by_currency,
                available_cash=broker_balance.available_cash,
                power=power,
                max_power_short=-1,
                net_cash_power=-1,
        ...
        # 如果该broker账户在数据库里有记录：
            ...
            for _, row in nondefault_position_df.iterrows():
                ...
                for idx, brk_pos in enumerate(all_broker_positions):
                    ...
                    if quantity > 0:
                        holding_price = (
                                                brk_pos.holding_price * brk_pos.quantity - db_pos.holding_price * db_pos.quantity) / quantity
                    elif quantity == 0:
    ...
    def get_history_data_by_dfield(self,
        ...
        if dtype is None:
            ...
            assert "time" in dtype[0] or "Time" in dtype[
                0], f"The first column in dtype should be related to `*time*`, but we got {dtype[0]}"
            time_col = dtype[0]
        data_cols = [col for col in df.columns if col != time_col]  # 除了time_col之外的所有其他数据
        data_cls = getattr(importlib.import_module("app.domain.data"), DATA_MODEL[dfield])
        datas = []
...
def convert_trade_market_db2qt(trade_market: str) -> TradeMarket:
    """将交易offset由string转换成Offset类"""
    if trade_market == "NONE":
        return TradeMarket.NONE
    elif trade_market == "HK":
        return TradeMarket.HK
    elif trade_market == "US":
        return TradeMarket.US
    elif trade_market == "CN":
        return TradeMarket.CN
    elif trade_market == "HKCC":
        return TradeMarket.HKCC
    elif trade_market == "FUTURES":
        return TradeMarket.FUTURES
    else:
        raise ValueError(f"TradeMarket {trade_market} in database is invalid!")
...
def persist_data(engine, time_interval=PERSIST_TIME_INTERVAL):
    ...
    db_main = engine.db
    # 在新线程里重新建立数据库连接
    DB = getattr(engine.plugins["mariadb"], "DB")
    setattr(engine, "db", DB())
    # 定期进行数据入库
    while engine.persist_active:
        persist_account_balance(engine)
        # logger.info("[Account balance] is persisted")
        persist_position(engine)
        # logger.info("[Position] is persisted")
        persist_order(engine)
        # logger.info("[Order] is persisted")
        persist_deal(engine)
        # logger.info("[Deal] is persisted")
        sleep(time_interval)
    engine.db.close()
    # 还原主线程的数据库连接
    setattr(engine, "db", db_main)
    logger.info("Gracefully stop persisting data.")
