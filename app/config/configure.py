...
class Configure:
    """Configure module will load a json file like `config.test.json` and parse the content to json object.
    ...
    def _update(self, update_fields) -> None:
        ...
        self.recorder = update_fields.get("RECORDER", {})
        self.rabbitmq = update_fields.get("RABBITMQ", None)
        self.db = update_fields.get("DB", None)
        self.redis = update_fields.get("REDIS", None)
        self.gateways = update_fields.get("GATEWAYS", None)
