...
from app.constants import Direction, TradeMode, Exchange, OrderType
...
from app.domain.security import Stock, Security, Currency, Futures
from app.domain.data import Bar, Quote, CapitalDistribution
from app.utils.utility import Time, try_parsing_datetime, BlockingDict
...
class IbGateway(BaseGateway):
    ...
    def req_market_data(
        ...
        """Request stores data (Quotes and Orderbooks)"""
        # Blocking here
        reqId = self.gen_valid_id()
    ...
    def subscribe(self):
        ...
        for security in self.securities:
            ...
            # Request stores data (quote and orderbook)
            ...
            if self.ib_quotes_done[security].wait():
                print(f"[{reqId}]Subscribed stores data for {security.code}")
